...
圖片支援PNG、JPEG、WebP（不支援動畫 WebP），最大 5 MB、最多 2000 萬像素且單邊不超過 10000 像素；後端在呼叫 AI 前檢查 base64、檔案標頭、大小及尺寸資訊，不在伺服器解壓縮圖片。內建相機會先縮圖；相簿圖片不會由後端重新壓縮。按「辨識圖片」，或選圖後使用照片開始解題，才送至後端及 Google Gemini；本站不把圖片寫入檔案或題庫。請拍清楚完整題目，不清楚的文字會提示重新核對。HEIC 等格式請先轉成 JPEG 或 PNG。
