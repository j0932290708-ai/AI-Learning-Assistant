...
function createPublicDemoAnswer() {
  const methodName = methodLabels[state.method] || state.method || '自動選擇';
  ...
  return {
    ...
    steps: [
      '辨認題目所屬單元，整理已知條件與要求。',
      `依題型選擇「${methodName}」，列出需要使用的觀念或公式。`,
      '逐步代入、檢查結果，最後整理成容易複習的答案。'
    ],
    answer: '這是免下載的公開展示版。八科選擇、解題方法、題目輸入、AI 繪圖與題庫儲存都可以直接操作；真正的 AI 解題需要另外連接安全的後端服務。',
    explanation: 'API 金鑰不能放在公開網頁中，因此展示網址不會暴露私人金鑰。'
  };
}
