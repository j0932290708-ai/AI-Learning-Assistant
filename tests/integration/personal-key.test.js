...
test('personal credentials are request-scoped, absent from prompts and never fall back to shared credentials', async () => {
  ...
  const make = key => ({models:{generateContent: async request => {
    ...
    return {text:JSON.stringify({subject:'english',method:'grammar',steps:['修正第三人稱'],answer:'She goes.'})};
  }}});
  ...
  try {
    ...
    assert.ok(seen.every(r=>JSON.parse(r.request.contents).question==='Correct: She go.'));
