...
目前 GitHub Pages 網址是互動示範。[正式 AI 版](https://ai-learning-assistant-pnma.onrender.com/) 由 Render 同時提供網頁、`/api/solve` 和 `/api/recognize`，沿用現有八科程式，無須額外資料庫。
...
圖片辨識已實作：以無個資的清晰題目圖片測試「上傳 → 辨識 → 檢查文字 → 填入題目 → 解題」，並確認換圖或移除圖片不會套用舊結果。本站不持久保存圖片，僅將資料送到 Gemini 辨識。另在 Android 與 iPhone 驗收相機、主畫面安裝與重開後的收藏。
